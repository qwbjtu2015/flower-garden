...
const DB_PATH = join(__dirname, 'flower_garden.db');
...
export async function initDb() {
  ...
  if (existsSync(DB_PATH)) {
    try {
      ...
      console.log(`[DB] 已从 ${DB_PATH} 加载, 大小: ${buffer.length} bytes`);
    } catch (e) {
  ...
  // 检查是否需要插入示例数据
  const result = db.exec('SELECT COUNT(*) as count FROM flowers');
  const flowerCount = result.length > 0 ? result[0].values[0][0] : 0;
...
export function saveDb() {
  if (db) {
    try {
      ...
      console.log(`[DB] 已保存到 ${DB_PATH}, 大小: ${buffer.length} bytes`);
    } catch (err) {
...
function insertSampleData() {
  ...
  for (const flower of flowers) {
    stmt.run([
      flower.name, flower.name_en, flower.category, flower.image,
      flower.description, flower.care_tips, flower.watering,
      flower.sunlight, flower.temperature, flower.fertilizer,
      flower.fertilizer_link, flower.difficulty, flower.season, flower.bloom_time
    ]);
  }

  stmt.free();
  console.log('已插入12种示例花卉数据');
}
...
export async function createAdminUser() {
  ...
  db.run(`INSERT INTO users (username, email, password, role) VALUES ('管理员', '${ADMIN_EMAIL}', '${hashedPassword}', 'admin')`);
  saveDb();
